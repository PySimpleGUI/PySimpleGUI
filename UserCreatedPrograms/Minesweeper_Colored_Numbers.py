...
# width       = 10    # Board Width     # Trinket Values
# height      = 8     # Board Height
# all         = 10    # BOMBS
width       = 25                        # Windows Values
height      = 14
all         = 80
new_start   = True
size        = (30,30)
...
class button():
    ...
    def update(self, state):        # update state of cell
        ...
        self.button.Update(text=text,
            disabled=self.disabled,
            image_data=im[self.state],
            image_size=size,
            button_color=c,
            disabled_button_color=(text_color,None))
...
window = sg.Window('MineSweeper', layout=layout, finalize=True, use_ttk_buttons=False) # Create window
